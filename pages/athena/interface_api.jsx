...
import { averageBlob, fetchWithTimeout, imgToBase64, normalize, replaceAll, shorten, similarText, splitStringWithPunctuation, toType } from "../utils";

export const HF_api_keys = [
...
function sum_prompt_array(p_array,mode){
    return p_array.map(i=>tokens[mode][i]||0.0).reduce((x,y)=>x+y,0).toFixed(3)
}

export function prompt_score(prompts,mode="imagine",min=6){
    var prompt_array = prompts.toLowerCase().replace(/\s+/g, " ").split(', ')
    var score = 0
    var options = Object.keys(tokens[mode]).filter(i=>prompt_array.includes(i)==false)
    let item = null
    while (score < min) {
...
export function toEndQuote(text){
    return normalize(text.split('”')[0].split('".')[0].split('."')[0].split('";')[0].split('"\n')[0])
}

export function uniqueWords(text){
    var sentence_array = text
    if(typeof(text)=='string') sentence_array = splitStringWithPunctuation(sentence_array)
    sentence_array = sentence_array.map(f=>normalize(f)).join(' ').split(' ').filter(a=>a!=''&&a.length>3)
    return sentence_array.filter((o,i)=>sentence_array.indexOf(o) == i)
}

export function scorePrompt(text,test_array){
    return text.split(' ').filter(a=>a!='').map(word=>test_array.includes(word)?1:0).reduce((a,b)=>a+b)
}
...
export async function depth_image(img){
    return new Promise(async(resolve, reject)=>{
        ...
        const response = await fetch("https://keras-io-monocular-depth-estimation.hf.space/api/predict",{
            headers: {
...
export async function image_inpainting(img, mask, prompt) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
            audio_data = await fetchWithTimeout("https://awacke1-text-to-speech-espnet-kan-bayash-2eb8bb8.hf.space/run/predict", {
            headers: {
                "Authorization": "Bearer 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",
                "X-Wait-For-Model":"true",
                "Content-Type": "application/json"
...
export async function compare_captions(img, base64=true){
    ...
    try {
        captions = await fetchWithTimeout("https://nielsr-comparing-captioning-models.hf.space/run/predict",{
            headers: {
                "X-Wait-For-Model":"true",
                "Content-Type": "application/json"
            },
...
export async function caption_image(img, prompts, onStep=(text)=>{console.log(text)}){
    ...
    // let inference = HF_api_keys[2]
    return new Promise(async(resolve, reject)=>{
        let captions = await compare_captions(img)
        if(captions?.data){
            onStep({state:"1-CaptionAI", data:captions.data})
            const filtered = (captions.data.sort((a,b)=>b.length-a.length)).filter((o,i)=> !i || similarText(o,captions.data[i-1]) <= 0.8)
            ...
            var promts_array = replaceAll(prompts.replace('/imagine',''),' ','_').split(',').filter(a=>a!=''&&a.length>=3)
            promts_array = (promts_array.filter((o,i)=>promts_array.indexOf(o) == i)).map((k)=>{return(replaceAll(k,'_',' '))})
            
            const score_array = promts_array.map(p=>scorePrompt(p,validate_array))
            // “”
            var prompts_short = promts_array.map((o,i)=>{return({text:o, score:score_array[i]})})
            .filter((o)=>o.score>0).sort((a,b)=>b.score-a.score).map(p=>p.text)
            // var prompts_short = promts_array.slice(0,Math.min(15,promts_array.length)).join(', ')
            prompts_short = (prompts_short.concat(validate_array.filter(v=>prompts_short.join(' ').includes(v) == false))).join(', ')
            ...
            const brain = {
                ...
                info:`The prompts that generated the picture:\n“${prompts_short}”.\nI have also recieved these ${filtered.length} descriptions, of this one photo, made by an AI about the image:\n“${filtered.join('”,\n“')}”;`,
                ...
                conclude:(a,b)=>`“${b}” this is my second option, if I where to combine with “${a}” this would be my final conclusion: “`
            }

            const query = `${brain.self}\n${brain.task}\n${brain.info}\n${brain.action}`
            await generate_text_stream(query, 128).then(response=>{
                final_text = toEndQuote(response.substr(query.length))
                onStep({state:"2-Descrition", data:final_text})
            }).catch((e)=>{ console.log("Error while summerizing text: "+e.message) })
            conclusion_text = final_text
            const validate = `${query} "${final_text}."\n${brain.review}`
            await generate_text_stream(validate, 128).then(response=>{
            ...
            if(similarText(final_text,validated_text) < 0.6){
                ...
                await generate_text_stream(conclude, 128).then(response=>{
            ...
            .then(final_comment=>{resolve(final_comment)})
            .catch((error)=>{console.log(error);resolve(conclusion_text)})
        }else{
...
export async function upscale_image(img){
    ...
    console.log(img_data_string)
    return new Promise(async(resolve, reject)=>{
        const response = await fetch("https://manjushri-sd-2x-and-4x-upscaler-gpu.hf.space/run/predict", {
            method: "POST",
...
export async function generate_text_stream(input,max=100,inference = HF_api_keys[1]){
...
export async function summerize_text(input, max=100,model='google/pegasus-large', inference = HF_api_keys[0]){
    return new Promise((resolve, reject)=>{
        ...
        .then((output)=>{resolve(output.summary_text)})
...
function getHighestOption(obj) {
    ...
    obj[0].labels.forEach((label, index) => {
      if (obj.reduce((acc, val) => acc + val.scores[index], 0) / obj.length > highestScore) {
        highestScore = obj.reduce((acc, val) => acc + val.scores[index], 0) / obj.length;
        highestLabel = label;
      }
    });
    return highestLabel;
...
export async function text_language_detection( inputs,
    inference = HF_api_keys[0]
){
...
export async function zero_shot_classification( inputs=[],
    ...
    return new Promise((resolve, reject)=>{
        ...
        inference.zeroShotClassification({
...
export async function generate_chat(){
    let inference = HF_api_keys[1]
...
export default async function generate_image(options={},onLoad=(image_blob)=>{ console.log(image_blob) },req_count=0){
    let inference = HF_api_keys[req_count%HF_api_keys.length]
    ...
    all_requests = config.images.map( async (image_data,index)=>{
        ...
        return inference.textToImage(query).then(async image_blob =>{
            await averageBlob(image_blob).then(async avg=>{
                if(avg == 0){
                    console.warn('Regenerating Emppty Image generation')
                    await get_image(query,req_count).then(image_blob =>{
...
export async function get_image(query,key=0){
    let inference = HF_api_keys[key%HF_api_keys.length]
    return await inference.textToImage(query).then(blob=>{return(blob)})
}
