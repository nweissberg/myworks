...
import { calculateResolution, isDeepEqual, moneyMask, print, roundToPower, toPower, useUtils } from "../../utils";
...
export default function EditorFooter(props) {
	...
	const [aspect_ratios, set_aspect_ratios] = useState([[1,1],[4,5],[3,4],[2,3],[9,16],[1,2]])
	...
	function regenerate(){
		...
		if(last_vision == null || isDeepEqual(last_vision,vision) == false){
	...
	useEffect(()=>{
		// print(vision)
		if(brain_data && props.imaginy_vision) set_models(Object.entries(brain_data.image.generate).map((model,index)=>{
			var _model = {...model[1], code: model[0], expanded:false, index:index}
			if(props.imaginy_vision.model?.model == model[0]) set_active_index( _model)
			return _model
		}))
	},[brain_data,vision])
	...
	function mobile_button(data){
		...
		return <div key={data.index}
				className={(data.className?data.className:'')+" flex flex-wrap justify-content-center py-2 max-w-4rem hover:bg-white-alpha-10 cursor-pointer "}
				onContextMenu={(e)=>{
					e.preventDefault()
					e.stopPropagation()
				}}
			...
			>
			<i className={`${data.icon} flex justify-content-center text-2xl w-4rem p-1`}/>
	...
	// 	minHeight:"100vh",
	// }
	let footer_class = " select-none flex flex-wrap surface-card w-full align-items-start animation-duration-200 animation-iteration-1 " +(orientation=='portrait'?'fadeindown': 'fadeinleft');
	var current_view = <></>
	const menu_footer_bar = ()=>{
		if(orientation == 'portrait'){
			current_view = <>
				<div key='footer_menu' className={(view=='menu'?'surface-card':'surface-50')+" z-1 h-4rem shadow-8 flex flex-wrap w-full align-items-start"}>
					<div className="flex w-screen justify-content-between overflow-scroll hide-scroll">{
			...
			current_view = <>
				<div key='footer_menu' className={(view=='menu'?'surface-card':'surface-ground')+" z-1 shadow-8 fixed top-0 left-0 h-screen w-4rem flex flex-wrap flex align-items-start"}>
				<div className="flex flex-wrap w-min h-screen justify-content-center overflow-scroll hide-scroll">{
						menu_buttons.map((d,i)=>mobile_button({...d, index:i}) )
					}</div>
				</div>
			</>
	...
	switch (view) {
		...
		case 'model': //Modelo
			current_view = <>
				...
				<div key={'footer_'+view}
					...
					<PanelHeader
					button={menu_buttons.find(data=>data.view == view)} onClick={()=>{
			...
			current_view = <>
				<div key={'footer_'+view}
					...
					style={orientation=='landscape'?{
						...
						height:"100%",
						minHeight:vertical?"256px":"222px",
					}}>
					<PanelHeader button={menu_buttons.find(data=>data.view == view)} onClick={regenerate}/>
					<div style={{top:"33px", width:'100%', position:'absolute'}} className="flex justify-content-center w-full">
						<div className={(orientation=='landscape'?'flex-wrap ':'')+"mt-3 z-2 flex w-30rem justify-content-between align-items-center"}>
							
							<Button
								label={!is_mobile || orientation=='landscape'?(()=>{
							...
							<div className={(orientation=='landscape'?'flex-wrap ':'')+"flex w-full justify-content-center gap-0 align-items-center h-full"}>
							...
							<Slider
								...
								onSlideEnd={(e)=>{
									update_vision(toPower(e.value), 'resolution')
								}}
								className="slider mx-3 mt-3 left-0 w-full "
							/>	
							<h4 className="whitespace-nowrap p-0 m-0 ">{toPower(resolution)} px</h4>
							</div>
						</div>
					</div>
					<div className="flex flex-wrap w-full h-min " >
						{orientation=='landscape'&&<div
							className=" w-max grid w-full mt-8 pt-5 ">
							{aspect_ratios.map((r,i)=>{
								var d = vertical ? r : [...r].reverse()
								return <div key={'ratio_'+i} className="flex flex-grow-1 w-full m-1 h-auto col-6 justify-content-center align-items-end">
									<Button
										label={<label className="font-bold text-lg text-black vertical-align-middle whitespace-nowrap">{`${d[0]} : ${d[1]}`}</label>}
										className={(vertical?"w-4rem":"h-4rem")+" text-black flex p-button-secondary border-round-sm justify-content-center  align-items-center p-0 m-0 " + (selected_ratio == i?'bg-white':'')}
						...
						{orientation=='portrait'&&<div
							className="flex gap-3 overflow-scroll hide-scroll px-3 align-items-center center justify-content-between w-auto max-w-screen-md">
							{aspect_ratios.map((r,i)=>{
								var d = vertical ? r : [...r].reverse()
								return <div key={'ratio_'+i}>
									<Button
										label={<label className="font-bold text-lg text-black vertical-align-middle whitespace-nowrap">{`${d[0]} : ${d[1]}`}</label>}
										className={(vertical?"w-4rem":"h-4rem")+" text-black flex p-button-secondary border-round-sm justify-content-center w-full align-items-center p-0 m-0 " + (selected_ratio == i?'bg-white':'')}
										style={{aspectRatio:`${d[0]}/${d[1]}`}}
										onClick={(e)=>{
											set_selected_ratio(i)
											update_vision(d, 'ratio')
		...
		case 'description':
			current_view = <>
				<div key={'footer_'+view}
					...
					/>
					{/* <InputTextarea valeu={vision?.description} onChange={(e)=>{
						update_vision(text,'description')
					}}/> */}
				</div>
			</>
		...
		case 'imagine':
			current_view = <>
				<div key={'footer_'+view} className={footer_class} style={{height:"100%", minHeight:"10vh"}}>
					<PanelHeader button={menu_buttons.find(data=>data.view == view)} onClick={regenerate}/>
		...
		case 'forget':
			current_view = <>
				<div key={'footer_'+view} className={footer_class + " hide-scroll overflow-scroll"} style={{height:"100%", minHeight:"10vh", maxHeight:"60vh"}}>
					<PanelHeader button={menu_buttons.find(data=>data.view == view)} onClick={regenerate}/>
					<AutoCompleteChips
						placeholder="Digite aqui ✍️"
						value={vision?.forget || ""}
		...
		default:
			current_view = <>
				<div key={'footer_'+view} className={footer_class} style={{height:"100%", minHeight:"40vh"}}>
					<div className="flex justify-content-between w-full">
						<h3>{menu_buttons.find(name=>name.view == view)?.label}</h3>
...
function PanelHeader(props){
	return<div className="flex justify-content-between align-items-center w-full sticky top-0 z-3 h-3rem surface-card px-3 shadow-8">
		
		<h3>{props.button?.label}</h3>
		<Button
			icon="pi pi-check font-bold"
			className="p-button-lg p-button-text shadow-none p-button-rounded p-3"
			{...props}
